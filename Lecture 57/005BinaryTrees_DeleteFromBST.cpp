...
TreeNode* deleteFromBST(TreeNode* root, int key) {
	...
	if (key == root->val) {
		...
		// recursively, delete the node with the given key from the leftSubtree which is also a BST
		root->left = deleteFromBST(root->left, key);
		...
		// recursively, delete the node with the given key from the rightSubtree which is also a BST
		root->right = deleteFromBST(root->right, key);
