...
public :
	...
	void dijkstra(T s) {
		...
		// time : VlogV (for extracting from minHeap) + ElogV (for relaxing tensed edges)

		// sparse graph i.e. E ~ V therfore time = VlogV + VlogV ~ O(VlogV)
		// dense graph i.e. E ~ V^2 therefore time = VlogV + V^2logV ~ O(V^2logV)

		// space : size of minHeap (unexplored) + size of set (explored) = O(V)

		while (!minHeap.empty()) {

			pair<int, T> minVertex = *minHeap.begin();
			minHeap.erase(minHeap.begin()); // logV
			...
			for (pair<T, int> neighbour : neighbourList) {
				...
				if (explored.find(neighbourLabel) == explored.end() &&
					...
					// update the distance of the neighbourLabel in the minHeap // ~logV
					auto it = minHeap.find({distMap[neighbourLabel], neighbourLabel}); // logV
					minHeap.erase(it); // logV
					minHeap.insert({distMap[minVertexLabel] + edgeWeight, neighbourLabel}); // logV
