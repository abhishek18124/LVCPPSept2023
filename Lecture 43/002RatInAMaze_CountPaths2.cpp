...
int f(char maze[][10], int m, int n, int i, int j) {
	...
	if (i == m || j == n) {
		...
		// you have gone outside the maze
	...
	if (i == m - 1 and j == n - 1) {
		...
		// you've reached the destination and 100% it is not blocked i.e. you've found a path
	...
	// count the #paths to go from (i, j) to (m-1, n-1)
