...
bool canPlace(int grid[][9], int n, int i, int j, int d) {
	...
	// check if digit 'd' is present in the ith row or jth column

	for (int k = 0; k < n; k++) {
		if (grid[i][k] == d || grid[k][j] == d) {
			// you cannot place digit 'd' in the (i, j)th cell of the grid
			return false;
		}
	}

	// check if digit 'd' is present in the subgrid of which (i, j)th cell is part of
	...
	for (int x = sx; x < sx + rn; x++) {
		for (int y = sy; y < sy + rn; y++) {
			if (grid[x][y] == d) {
				// you've found 'd' in the subgrid of (i, j)th cell therefore you cannot assign to grid[i][j]
				return false;
			}
		}
	}
