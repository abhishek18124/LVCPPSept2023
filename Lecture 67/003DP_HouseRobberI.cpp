...
int helperTopDown(vector<int>& nums, int n, int i, vector<int>& dp) {
    ...
    if (dp[i] != -1) {
        ...
        // you've already solve f(i) previously
...
// time : O(n)
// space: O(n) due to dp[], try to optimise the space to O(1)
...
int helperBottomUp(vector<int> nums, int n) {
    ...
    dp[n + 1] = dp[n] = 0; // at the nth index and n+1th index of dp[], we store f(n), f(n+1)

    for (int i = n - 1; i >= 0; i--) {

        // dp[i] = f(i)

        // decide the ith house
