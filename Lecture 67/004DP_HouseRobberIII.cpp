...
int helper(TreeNode* root) {
    ...
    if (root == NULL) {
        ...
        // tree is empty, there are no houses
    ...
    // option 1 : don't rob at the root node

    int dontRobOption = helper(root->left) + helper(root->right);

    // option 2 : rob at the root node
...
int helperTopDown(TreeNode* root, unordered_map<TreeNode*, int>& dp) {
    ...
    if (dp.find(root) != dp.end()) {
        ...
        // you already know the maximum money you can rob from the subtree rooted at root
